...
| `min_smart_money_wallets` | integer | ❌ | **5** | 最小聪明钱钱包数（触发入场），**已优化：从10降到5**，更早捕捉信号 |
...
## V1.0.1 - 优化版本（2026-05-14）

### 优化内容

1. **降低入场阈值**：`min_smart_money_wallets` 从 10 降到 **5**
   - 原因：报告显示钱包数≥10时市值已在高点，代币已运行一段时间
   - 效果：更早捕捉信号，提高入场成功率

2. **增加流动性过滤**：新增 `MIN_LIQUIDITY = $25,000`
   - 原因：低流动性代币价格波动极大，容易滑点
   - 实现：调用 `onchainos token liquidity` 检查流动性
   - 效果：过滤掉流动性不足的代币，降低风险

3. **过滤极低市值代币**：新增 `MIN_MARKET_CAP = $20,000`
   - 原因：市值<$20K的代币通常流动性极差，价格波动剧烈
   - 效果：避免在极端高风险代币上交易

4. **实现止损机制**：
   - 固定止损：-20%
   - 分层止盈：2x/5x/10x
   - 移动止损：盈利≥50%后激活，回撤10%止损
   - 实现：在 `solana-smart-money-monitor.py` 中添加止损检查逻辑

### 修改的文件

| 文件 | 修改内容 |
|------|----------|
| `solana-smart-money-monitor.py` | 降低 threshold、增加流动性检查函数、添加止损逻辑 |
| `smart-money-unified.sh` | 降低 MIN_WALLETS、增加流动性检查、过滤低市值 |
| `smart-money-simulation.sh` | 同步优化参数 |
| `SKILL.md` | 更新参数说明、添加 V1.0.1 版本说明 |
...
6. 本策略使用 onchainOS 作为主要信息源和交易工具
