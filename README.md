...
**聪明钱交易者** 是一款基于链上聪明钱信号驱动的 DEX 激进交易策略，基于 [onchainOS](https://github.com/okx/onchainos) 追踪 Solana 链上聪明钱钱包的实时买入信号，结合动量确认和 13 项安全检查，执行激进但可控的现货交易。
...
> 💡 本策略基于 onchainOS 构建，充分利用链上数据与 DEX 交易能力。

### 核心优势

- **完整策略流程**：从信号采集到持仓监控，8个步骤全覆盖
- **13项安全检查**：每笔交易前强制通过安全检查
- **动态仓位管理**：根据账户净值动态调整仓位
- **分层止盈 + 移动止损**：兼顾利润锁定与上行空间
